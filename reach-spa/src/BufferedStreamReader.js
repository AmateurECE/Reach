 ...
 * Skipping/hissing occurs if buffer is too small or if network isn't fast
 * enough.
 * Users must wait too long to hear audio if buffer is too large.
 *
 * Returns Promise that resolves when entire stream is read and bytes queued
 * for decoding
 */

// Callbacks:
// onRead: { bytes, totalRead, totalBytes, done }
// onBufferFull: { bytes, done }
class BufferedStreamReader {
    ...
    async _start() {
        ...
        if (!response.body) throw Error(
            'ReadableStream not yet supported in this browser - <a href="'
                + 'https://developer.mozilla.org/en-US/docs/Web/API/Body/body'
                + '#Browser_Compatibility">browser compatibility</a>');
        
        // requires CORS access-control-expose-headers: content-length
        ...
        const read = async () => {
            ...
            // avoid blocking read()
            setTimeout(_ => this._readIntoBuffer({
                value, done, request: this.request}));
...
export default BufferedStreamReader;
